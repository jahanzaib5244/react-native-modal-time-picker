import { runOnJS, useAnimatedGestureHandler, useAnimatedStyle, useSharedValue, withDecay, withSpring } from 'react-native-reanimated';
...
export default function useTimePicker(handelChange, onPressConfirm) {
  ...
  useEffect(() => {
    ...
    hoursY.value = withSpring(-currentHourIndex * 50, {
      duration: 100
    });
    minutesY.value = withSpring(-currentMinuteIndex * 50, {
      duration: 100
    });
    typeY.value = withSpring(-currentFormatIndex * 50, {
      duration: 100
    });
  }, []);
  const onConfirm = () => {
  ...
  function calculateDecay(animationValue, array, velocityY) {
    ...
    animationValue.value = withDecay({
      ...
      if (Math.abs(currentValue) <= (array.length - 1) * 50) {
        animationValue.value = withSpring(Math.round(currentValue / 50) * 50, {
          duration: 10
        }, () => {
          runOnJS(handleItemChange)();
        });
      } else {
        animationValue.value = withSpring(-(array.length - 1) * 50, {
          duration: 10
        }, () => {
          runOnJS(handleItemChange)();
        });
